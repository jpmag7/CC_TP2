...
public class PacketHandler extends Thread
{
    ...
    public void handlePacket() throws Exception{
        ...
        switch(id){
            case SystemInfo.REQUEST:
                receiveRequest();
                break;
            case SystemInfo.SHUTDOWN:
                SystemInfo.doneClients.add(address);
                System.out.println("Receiving shutdown signal .-.-.-.-.-.-.-.-.-.-.-.-");
                PacketUtil.send(socket, address, SystemInfo.FTRapidPort, new byte[0], SystemInfo.SHUTDOWNACK);
                Listener.checkIfAllDone();
                break;
            case SystemInfo.SHUTDOWNACK:
                SystemInfo.fileLowestMissing.get(address).put(SystemInfo.SHUTDOWN, 0);
                System.out.println("Receiving shutdown signal ACK .-.-.-.-.-.-.-.-.-.-.-.-");
                SystemInfo.fileTimers.get(address).put(SystemInfo.SHUTDOWN, SystemInfo.BatchWaitTime + 1L);
    ...
    private void sendListBatch(int sequence, int batchSizeSend) throws Exception{
        ...
        System.out.println("List batch request received. start: " + start + " end: " + end);
    ...
    private void receiveResponse(int file) throws Exception{ // The number of the file we are receiving
        ...
        if(seq < 0){
            ...
            System.out.println("ERROR: sequence -1 code ---------------------------------------");
            l.unlock();
            return;
        }
        ...
        if(!fileSeq.contains(seq)){ // If we dont have already have this sequence
            ...
            if(lowestMissingSeq == total){ // We received the entire file             Handle entire file ending here ---------------------|+|
                System.err.println("Full file received: " + file);
        ...
        for(Map.Entry<InetAddress, Map<Integer, String>> tl : SystemInfo.their_lists.entrySet()){
            for(Map.Entry<Integer, String> e : tl.getValue().entrySet()){
                if(!list.contains(e.getValue())){
                    ...
                    new Requester(socket, tl.getKey(), SystemInfo.FTRapidPort, e.getKey(), SystemInfo.REQUEST).start();
                }
